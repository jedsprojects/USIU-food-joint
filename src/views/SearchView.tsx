...
export default function SearchView({ onNavigate }: Props) {
  ...
  return (
    <main className="search-view app-view app-view--inset fade-in-up">
      ...
      {/* Search Input */}
      <div className="search-input-wrap glass-panel" style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', borderRadius: 'var(--radius-xl)', marginBottom: '24px', border: '1px solid rgba(200, 150, 168, 0.15)' }}>
        <span className="material-symbols-outlined" style={{ color: 'var(--color-on-surface-variant)', marginRight: '12px' }}>search</span>
        <input 
          type="text" 
          placeholder="Search burgers, wraps, smocha, fries..." 
          value={query}
      ...
      {/* Filters Sheet / Panel */}
      <div className="filters-panel glass-panel" style={{ padding: '16px', borderRadius: 'var(--radius-card)', marginBottom: '24px', border: '1px solid rgba(200, 150, 168, 0.1)' }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            ...
            <span className="font-price-display" style={{ color: 'var(--color-tertiary-fixed)', fontSize: '14px' }}>Ksh {maxPrice}</span>
          </div>
          <input 
            type="range" 
            min="50" 
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="product-card" onClick={() => onNavigate('detail', product)} role="button" tabIndex={0}>
              <div className="product-card__img-wrap">
                ...
                {product.isDrop && <span className="product-card__drop-badge">ðŸ”¥</span>}
              </div>
