...
const DELIVERY_TIMES = ['ASAP (~35 mins)', '12:15 PM â€“ 12:45 PM', '12:45 PM â€“ 1:15 PM', '1:15 PM â€“ 1:45 PM', '6:15 PM â€“ 6:45 PM', '6:45 PM â€“ 7:15 PM'];
...
export default function PreOrderView({ onNavigate }: Props) {
  ...
  const { userProfile, updateUserProfile, getOrderDisplayName, getOrderPhone } = useAuth();
  
  const [guestName, setGuestName] = useState(() => getOrderDisplayName());
  const [guestPhone, setGuestPhone] = useState(() => getOrderPhone());
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!guestName.trim()) {
      addToast('Please enter your name', 'error', 'error');
      return;
    }
    if (!guestPhone.trim()) {
      addToast('Please enter your phone number', 'error', 'error');
      return;
    }

    try {
      // Save contact details to profile first
      await updateUserProfile({
        guestDisplayName: guestName.trim(),
        guestPhone: guestPhone.trim(),
        displayName: userProfile?.isAnonymous ? undefined : guestName.trim(),
        phone: userProfile?.isAnonymous ? undefined : guestPhone.trim(),
      });
      ...
      if (orderType === 'delivery') {
        const found = activeAddresses.find(a => a.id === selectedAddressId);
        if (found) {
          finalAddress = `${found.address}, ${found.city}`;
        }
      }
  ...
  return (
    <main className="preorder-view app-view app-view--inset fade-in-up">
      ...
      {/* Contact Information */}
      <div style={{ marginBottom: '24px' }}>
        <h3 className="font-headline-md" style={{ color: 'var(--color-on-surface)', fontSize: '18px', marginBottom: '12px' }}>Contact Information</h3>
        <div className="glass-panel" style={{ padding: '16px', borderRadius: 'var(--radius-card)', border: '1px solid rgba(200, 150, 168, 0.15)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div>
            <label className="font-label-md" style={{ color: 'var(--color-on-surface-variant)', display: 'block', marginBottom: '6px', fontSize: '11px', fontWeight: 'bold', letterSpacing: '0.05em' }}>YOUR NAME</label>
            <input 
              type="text" 
              placeholder="Enter your name" 
              value={guestName} 
              onChange={e => setGuestName(e.target.value)}
              style={{ width: '100%', background: 'var(--color-surface-container-lowest)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: 'var(--radius-xl)', padding: '12px 14px', color: '#fff', fontSize: '14px', outline: 'none' }}
            />
          </div>
          <div>
            <label className="font-label-md" style={{ color: 'var(--color-on-surface-variant)', display: 'block', marginBottom: '6px', fontSize: '11px', fontWeight: 'bold', letterSpacing: '0.05em' }}>PHONE NUMBER (FOR WHATSAPP/CALL)</label>
            <input 
              type="tel" 
      ...
      {/* Mode Selector */}
      <div className="glass-panel" style={{ display: 'flex', padding: '6px', borderRadius: 'var(--radius-full)', marginBottom: '24px', border: '1px solid rgba(200, 150, 168, 0.15)' }}>
        <button 
          ...
          {(orderType === 'pickup' ? PICKUP_TIMES : DELIVERY_TIMES).map(time => (
            <button
              ...
              style={{
                ...
                border: scheduledTime === time ? '1px solid var(--color-primary)' : '1px solid rgba(200, 150, 168, 0.05)',
                fontWeight: 'bold',
                fontSize: '12px',
                textAlign: 'center',
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p className="font-body-md" style={{ color: 'var(--color-on-surface-variant)', fontSize: '14px', marginBottom: '16px' }}>
          Pay via M-Pesa Paybill / Till <strong>0742189696</strong> and attach the screenshot for faster approval.
        </p>
      ...
      <div className="cart-summary" style={{ background: 'var(--color-primary-container)', borderRadius: 'var(--radius-card)', padding: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
          ...
          <span className="font-price-display" style={{ color: 'var(--color-on-surface)' }}>Ksh {cartTotal}</span>
        </div>
        {promoDiscount > 0 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <span className="font-label-md" style={{ color: 'var(--color-tertiary)' }}>Promo Discount</span>
            <span className="font-price-display" style={{ color: 'var(--color-tertiary)' }}>−Ksh {promoDiscount}</span>
          </div>
        )}
        {orderType === 'delivery' && (
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', borderBottom: '1px solid rgba(200, 150, 168, 0.1)', paddingBottom: '8px' }}>
            <span className="font-label-md" style={{ color: 'var(--color-on-primary-container)' }}>Delivery Fee</span>
            <span className="font-price-display" style={{ color: 'var(--color-on-surface)' }}>Ksh {deliveryFee}</span>
          </div>
        )}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '16px 0 20px 0' }}>
          ...
          <span className="font-price-display" style={{ color: 'var(--color-on-surface)', fontSize: '24px' }}>Ksh {grandTotal}</span>
        </div>
