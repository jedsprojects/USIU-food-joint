...
export default function OrderTrackingView({ onNavigate }: Props) {
  ...
  const { user, getOrderDisplayName } = useAuth();
  const customerId = user ? user.uid : 'guest-temp';
  const thread = messages.find(m => m.customerId === customerId || m.userId === customerId);

  const handleSendChat = (e: React.FormEvent) => {
    e.preventDefault();
    if (!chatText.trim()) return;
    sendMessage(customerId, chatText, 'customer', getOrderDisplayName());
    setChatText('');
  };
  ...
  return (
    <main className="order-tracking-view app-view app-view--inset fade-in-up">
      ...
      <div className="glass-panel" style={{ 
        ...
        {order.type === 'pickup' ? (
          <div>
            <span className="badge badge--gold" style={{ marginBottom: '8px' }}>ðŸš¶ Pickup Slot</span>
            {order.status === 'ready' ? (
          ...
          <div>
            <span className="badge badge--gold" style={{ marginBottom: '8px' }}>ðŸš´ Delivery Mode</span>
            ...
            <p className="font-body-md" style={{ color: 'var(--color-on-surface-variant)', marginTop: '8px' }}>
              {order.status === 'delivered' 
                ? 'Thank you for dining with Kwa Gavo. Fresh. Fast. Fire! 🔥' 
                : 'Your order is being prepared with care. Hold tight!'}
            </p>
          </div>
        )}
      </div>
      ...
      {order.type === 'delivery' && (
        <div className="glass-panel" style={{ 
          ...
          <svg style={{ width: '100%', height: '100%' }}>
            {/* Curved Path */}
            <path d="M 40 120 Q 150 40, 200 120 T 360 60" fill="transparent" stroke="rgba(200, 150, 168, 0.2)" strokeWidth="4" strokeDasharray="6, 6" />
