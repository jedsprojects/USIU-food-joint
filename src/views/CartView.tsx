  ...
  return (
    <main className="cart-view app-view fade-in-up">
      <div className="cart-header">
        <h1 className="font-headline-lg-mobile" style={{ color: 'var(--color-on-surface)' }}>Your Order</h1>
        <p className="font-label-md" style={{ color: 'var(--color-on-surface-variant)', marginTop: '4px' }}>Review your selection before checkout</p>
      </div>
        ...
        {cart.map(({ product, quantity }) => (
          <div key={product.id} className="cart-item glass-panel">
            ...
            <div className="cart-item__controls">
              <span className="font-price-display" style={{ color: 'var(--color-tertiary-fixed)' }}>Ksh {product.price * quantity}</span>
      ...
      <div className="promo-section">
        {promoCode ? (
          <div className="promo-applied">
            ...
            <span className="font-label-md" style={{ color: 'var(--color-tertiary)' }}>{promoCode} applied (−Ksh {promoDiscount})</span>
          ...
          <div className="upsell-scroll scroll-hide">
            {upsells.map(p => (
              <div key={p.id} className="upsell-card" onClick={() => onNavigate('detail', p)}>
                ...
                <span className="font-label-md" style={{ color: 'var(--color-tertiary-fixed)', fontSize: '12px' }}>Ksh {p.price}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="cart-summary">
        ...
        <div className="cart-summary__total">
          <div>
            ...
            <h2 className="font-headline-xl" style={{ color: 'var(--color-on-surface)' }}>Ksh {grandTotal}</h2>
          </div>
        </div>

        {/* Loyalty Incentive Callout */}
        <div className="cart-loyalty-callout">
          <span className="material-symbols-outlined fill-1 cart-loyalty-callout__icon">workspace_premium</span>
          <div className="cart-loyalty-callout__text">
            <p className="cart-loyalty-callout__headline">Earn Loyalty Points</p>
            <p className="cart-loyalty-callout__body">Every app order earns you points toward <strong>free meals</strong> & exclusive perks!</p>
          </div>
        </div>

        <button className="cart-checkout-btn ripple-btn" onClick={() => onNavigate('preorder')}>
          <span className="font-label-md" style={{ textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.1em' }}>Continue to Order</span>
